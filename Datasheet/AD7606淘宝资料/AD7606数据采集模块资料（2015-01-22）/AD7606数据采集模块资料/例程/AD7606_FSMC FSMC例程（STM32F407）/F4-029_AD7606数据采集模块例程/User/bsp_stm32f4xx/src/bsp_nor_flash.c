...
*	版    本 : V1.0
*	说    明 : 安富莱STM32-F4开发板标配的NOR Flash 为 S29GL128P10TFI01  容量16M字节，16Bit，100ns速度
*				物理地址 : 0x6400 0000
*	修改记录 :
*		版本号  日期        作者     说明
...
*	功能说明: 页面内编程（32字节一个页面）. 编程前需要保证存储单元是全0xFF状态。可以重复编程相同的数据。
*	形    参: 	pBuffer : 数据存放在此缓冲区
*				_uiWriteAddr : 偏移地址[0, 16*1024*1024 - 2]; 编程地址必须为偶数
*				_usNumHalfword      : 数据格式，双字节为1个单位
*
*	返 回 值: NOR_SUCCESS, NOR_ERROR, NOR_TIMEOUT
*********************************************************************************************************
*/
